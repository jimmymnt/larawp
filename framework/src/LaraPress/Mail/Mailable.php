...
use LaraPress\Contracts\Filesystem\Factory as FilesystemFactory;
use LaraPress\Contracts\Mail\Factory as MailFactory;
use LaraPress\Contracts\Mail\Mailable as MailableContract;
use LaraPress\Contracts\Queue\Factory as Queue;
...
class Mailable implements MailableContract, Renderable
{
     ...
     *
     * @param \LaraPress\Contracts\Mail\Factory|\LaraPress\Contracts\Mail\Mailer $mailer
     * @return void
     */
     ...
     *
     * @param \LaraPress\Mail\Markdown $markdown
     * @param array $data
     * @return string
     */
     ...
     *
     * @param \LaraPress\Mail\Message $message
     * @return $this
     */
    ...
    protected function buildSubject($message)
    {
        if ($this->subject) {
            ...
            $message->subject(Str::title(Str::snake(lp_class_basename($this), ' ')));
        }
    ...
    protected function hasRecipient($address, $name = null, $property = 'to')
    {
        ...
        return lp_collect($this->{$property})->contains(function ($actual) use ($expected) {
    ...
    {
        $this->attachments = lp_collect($this->attachments)
    ...
    {
        $this->diskAttachments = lp_collect($this->diskAttachments)->push([
            'disk' => $disk,
            'path' => $path,
    ...
    {
        $this->rawAttachments = lp_collect($this->rawAttachments)

...
class Stringable implements JsonSerializable
{
    ...
    {
        return new static(lp_class_basename($this->value));
    }
    ...
    {
        return lp_collect(explode($delimiter, $this->value, $limit));
    }
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (filter_var($pattern, FILTER_VALIDATE_INT) !== false) {
            return lp_collect(mb_str_split($this->value, $pattern));
        }

        $segments = preg_split($pattern, $this->value, $limit, $flags);

        return !empty($segments) ? lp_collect($segments) : lp_collect();
    }
    ...
    {
        return lp_collect(sscanf($this->value, $format));
    }
    ...
    {
        return lp_collect(Str::ucsplit($this->value));
    }

...
class Batch implements Arrayable, JsonSerializable
{
     ...
     *
     * @var \LaraPress\Bus\BatchRepository
     */
     ...
     *
     * @param \LaraPress\Support\Enumerable|array $jobs
     * @return self
     */
    public function add($jobs)
    {
        ...
        $jobs = Collection::wrap($jobs)->map(function ($job) use (&$count) {
            ...
            if (is_array($job)) {
                ...
                return lp_with($this->prepareBatchedChain($job), function ($chain) {
                    return $chain->first()
    ...
    public function recordSuccessfulJob(string $jobId)
    {
        ...
        if ($counts->pendingJobs === 0 && $this->hasThenCallbacks()) {
            ...
            lp_collect($this->options['then'])->each(function ($handler) use ($batch) {
        ...
        if ($counts->allJobsHaveRanExactlyOnce() && $this->hasFinallyCallbacks()) {
            ...
            lp_collect($this->options['finally'])->each(function ($handler) use ($batch) {
                $this->invokeHandlerCallback($handler, $batch);
            });
        }
    }
     ...
     *
     * @param string $jobId
     * @return \LaraPress\Bus\UpdatedBatchJobCounts
     */
    ...
    public function recordFailedJob(string $jobId, $e)
    {
        ...
        if ($counts->failedJobs === 1 && $this->hasCatchCallbacks()) {
            ...
            lp_collect($this->options['catch'])->each(function ($handler) use ($batch, $e) {
        ...
        if ($counts->allJobsHaveRanExactlyOnce() && $this->hasFinallyCallbacks()) {
            ...
            lp_collect($this->options['finally'])->each(function ($handler) use ($batch, $e) {
                $this->invokeHandlerCallback($handler, $batch, $e);
            });
        }
    }

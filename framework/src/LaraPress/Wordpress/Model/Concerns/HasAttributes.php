...
use LaraPress\Contracts\Database\Eloquent\CastsInboundAttributes;
use LaraPress\Contracts\Support\Arrayable;
use LaraPress\Database\Eloquent\Casts\AsArrayObject;
use LaraPress\Database\Eloquent\Casts\AsCollection;
use LaraPress\Database\Eloquent\Casts\Attribute;
use LaraPress\Database\Eloquent\InvalidCastException;
use LaraPress\Database\Eloquent\JsonEncodingException;
use LaraPress\Database\Eloquent\Relations\Relation;
use LaraPress\Database\LazyLoadingViolationException;
use LaraPress\Support\Arr;
use LaraPress\Support\Carbon;
use LaraPress\Support\Collection as BaseCollection;
use LaraPress\Support\Facades\Crypt;
use LaraPress\Support\Facades\Date;
use LaraPress\Support\Str;
use function LaraPress\Wordpress\Auth\Concerns\enum_exists;
use const LaraPress\Wordpress\Auth\Concerns\PHP_FLOAT_EPSILON;
...
trait HasAttributes
{
    ...
    protected function getRelationshipFromMethod($method)
    {
        ...
        return lp_tap($relation->getResults(), function ($results) use ($method) {
    ...
    {
        return lp_tap($this->getArrayAttributeByKey($key), function (&$array) use ($path, $value) {
     ...
     * @return bool
     *
     * @throws \LaraPress\Database\Eloquent\InvalidCastException
     */
    ...
    protected function getOriginalWithoutRewindingModel($key = null, $default = null)
    {
        if ($key) {
            return $this->transformModelValue(
                $key, lp_data_get($this->originalData, $key, $default)
            );
        }

        return lp_collect($this->originalData)->mapWithKeys(function ($value, $key) {
    ...
    public function originalIsEquivalent($key)
    {
        ...
        $attribute = lp_data_get($this->data, $key);
        $original = lp_data_get($this->originalData, $key);
    ...
    public static function cacheMutatedAttributes($class)
    {
        static::$getAttributeMutatorCache[$class] =
            lp_collect($attributeMutatorMethods = static::getAttributeMarkedMutatorMethods($class))
                ->mapWithKeys(function ($match) {
                    return [lcfirst(static::$snakeAttributes ? Str::snake($match) : $match) => true];
                })->all();

        static::$mutatorCache[$class] = lp_collect(static::getMutatorMethods($class))
    ...
    protected static function getAttributeMarkedMutatorMethods($class)
    {
        ...
        return lp_collect((new ReflectionClass($instance))->getMethods())->filter(function ($method) use ($instance) {

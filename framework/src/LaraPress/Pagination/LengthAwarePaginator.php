...
use ArrayAccess;
use Countable;
use LaraPress\Contracts\Pagination\LengthAwarePaginator as LengthAwarePaginatorContract;
...
class LengthAwarePaginator extends AbstractPaginator implements Arrayable, ArrayAccess, Countable, IteratorAggregate, Jsonable, JsonSerializable, LengthAwarePaginatorContract
{
     ...
     * @param array $data
     * @return \LaraPress\Contracts\Support\Htmlable
     */
    ...
    {
        return lp_collect($this->elements())->flatMap(function ($item) {
            if (!is_array($item)) {
                return [['url' => null, 'label' => '...', 'active' => false]];
            }

            return lp_collect($item)->map(function ($url, $page) {
                return [
                    'url' => $url,
                    'label' => (string)$page,
            ...
            'label' => function_exists('lp___') ? lp___('pagination.previous') : 'Previous',
            'active' => false,
        ])->push([
            'url' => $this->nextPageUrl(),
            'label' => function_exists('lp___') ? lp___('pagination.next') : 'Next',
            'active' => false,
        ]);
    }

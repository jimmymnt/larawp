...
namespace LaraPress\Database\Query\Grammars;

use LaraPress\Database\Grammar as BaseGrammar;
use LaraPress\Database\Query\Builder;
...
class Grammar extends BaseGrammar
{
    ...
    {
        return lp_collect($joins)->map(function ($join) use ($query) {
    ...
    {
        return lp_collect($query->wheres)->map(function ($where) use ($query) {
    ...
    protected function compileHavingBetween($having)
    {
        ...
        $min = $this->parameter(lp_head($having['values']));

        $max = $this->parameter(lp_last($having['values']));
    ...
    public function compileInsert(Builder $query, array $values)
    {
        ...
        $parameters = lp_collect($values)->map(function ($record) {
    ...
    {
        return lp_collect($values)->map(function ($value, $key) {
    ...
    {
        $alias = lp_last(explode(' as ', $table));
     ...
     *
     * @param \LaraPress\Database\Query\Expression|string $value

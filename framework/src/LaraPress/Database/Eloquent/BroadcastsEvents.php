...
trait BroadcastsEvents
{
     ...
     *
     * @param \LaraPress\Broadcasting\Channel|\LaraPress\Contracts\Broadcasting\HasBroadcastChannel|array|null $channels
     ...
     * @return \LaraPress\Broadcasting\PendingBroadcast|null
     */
    protected function broadcastIfBroadcastChannelsExistForEvent($instance, $event, $channels = null)
    {
        ...
        if (!empty($this->broadcastOn($event)) || !empty($channels)) {
            return lp_broadcast($instance->onChannels(Arr::wrap($channels)));
        }
    }
    ...
    {
        return lp_tap($this->newBroadcastableEvent($event), function ($event) {
     ...
     *
     * @param string $event
     * @return \LaraPress\Database\Eloquent\BroadcastableModelEventOccurred
     */
     ...
     *
     * @param string $event
     * @return \LaraPress\Broadcasting\Channel|array
     */

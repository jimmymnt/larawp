...
namespace LaraPress\Validation\Concerns;
...
use LaraPress\Validation\ValidationData;
...
trait ValidatesAttributes
{
    ...
    public function validateEmail($attribute, $value, $parameters)
    {
        ...
        $validations = lp_collect($parameters)
            ->unique()
    ...
    public function guessColumnForQuery($attribute)
    {
        if (in_array($attribute, Arr::collapse($this->implicitAttributes))
            && !is_numeric($last = lp_last(explode('.', $attribute)))) {
            return $last;
        }

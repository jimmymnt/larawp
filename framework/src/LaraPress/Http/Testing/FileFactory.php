...
namespace LaraPress\Http\Testing;
...
class FileFactory
{
     ...
     * @return \LaraPress\Http\Testing\File
     */
    public function create($name, $kilobytes = 0, $mimeType = null)
    {
        ...
        return lp_tap(new File($name, tmpfile()), function ($file) use ($kilobytes, $mimeType) {
    ...
    public function createWithContent($name, $content)
    {
        ...
        return lp_tap(new File($name, $tmpfile), function ($file) use ($tmpfile) {
    ...
    {
        return lp_tap(tmpfile(), function ($temp) use ($width, $height, $extension) {
            ob_start();

...
namespace LaraPress\Http\Concerns;
...
trait InteractsWithInput
{
    ...
    {
        if ($this->has($key)) {
            return $callback(lp_data_get($this->all(), $key)) ?: $this;
        }
     ...
     * @return \LaraPress\Support\Carbon|null
     */
    ...
    {
        return lp_collect(is_array($key) ? $this->only($key) : $this->input($key));
    }
    ...
    public function only($keys)
    {
        ...
        foreach (is_array($keys) ? $keys : func_get_args() as $key) {
            $value = lp_data_get($input, $key, $placeholder);
     ...
     * @param string|null $key
     * @param mixed $default
     * @return \LaraPress\Http\UploadedFile|\LaraPress\Http\UploadedFile[]|array|null
     */
    public function file($key = null, $default = null)
    {
        return lp_data_get($this->allFiles(), $key, $default);
    }

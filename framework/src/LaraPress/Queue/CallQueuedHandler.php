...
use LaraPress\Contracts\Encryption\Encrypter;
use LaraPress\Contracts\Queue\Job;
use LaraPress\Contracts\Queue\ShouldBeUnique;
use LaraPress\Contracts\Queue\ShouldBeUniqueUntilProcessing;
use LaraPress\Database\Eloquent\ModelNotFoundException;
use LaraPress\Pipeline\Pipeline;
...
class CallQueuedHandler
{
     ...
     *
     * @param \LaraPress\Contracts\Bus\Dispatcher $dispatcher
    ...
    {
        if (in_array(InteractsWithQueue::class, lp_class_uses_recursive($instance))) {
            $instance->setJob($job);
        }
    ...
    {
        $uses = lp_class_uses_recursive($command);
    ...
    {
        if (!in_array(Batchable::class, lp_class_uses_recursive($command)) ||
            is_null($command->batch())) {
            return;
        }

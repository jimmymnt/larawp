...
class AssertableJsonString implements ArrayAccess, Countable
{
     ...
     *
     * @var \LaraPress\Contracts\Support\Jsonable|\JsonSerializable|array
     */
    public $json;
     ...
     *
     * @param \LaraPress\Contracts\Support\Jsonable|\JsonSerializable|array|string $jsonable
     * @return void
     */
    ...
    {
        return lp_data_get($this->decoded, $key);
    }
    ...
    public function assertCount(int $count, $key = null)
    {
        if (!is_null($key)) {
            PHPUnit::assertCount(
                $count, lp_data_get($this->decoded, $key),

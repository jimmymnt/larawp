    ...
    public function __construct($timezone = null)
    {
        ...
        if (!class_exists(Container::class)) {
            throw new RuntimeException(
                'A container implementation is required to use the scheduler. Please install the larawp/container package.'
            );
        }
    ...
    protected function dispatchToQueue($job, $queue, $connection)
    {
        if ($job instanceof Closure) {
            if (!class_exists(CallQueuedClosure::class)) {
                throw new RuntimeException(
                    'To enable support for closure jobs, please install the larawp/queue package.'
                );
            }
    ...
    protected function getDispatcher()
    {
        if ($this->dispatcher === null) {
            try {
                ...
                throw new RuntimeException(
                    'Unable to resolve the dispatcher from the service container. Please bind it or install the larawp/bus package.',
                    $e->getCode(), $e
                );
            }
        }

...
use LaraWP\Contracts\View\Factory;
use LaraWP\Filesystem\Filesystem;
use LaraWP\Support\Str;
use LaraWP\View\AnonymousComponent;
use LaraWP\View\DynamicComponent;
use LaraWP\View\ViewFinderInterface;
...
class ComponentTagCompiler
{
     ...
     *
     * @var \LaraWP\View\Compilers\BladeCompiler
     */
    protected $blade;
     ...
     * @param \LaraWP\View\Compilers\BladeCompiler|null $blade
     * @return void
     */
...
<?php $component->withAttributes([' . $this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class) . ']); ?>';
    ...
    public function partitionDataAndAttributes($class, array $attributes)
    {
        ...
        if (!class_exists($class)) {
            return [lp_collect($attributes), lp_collect($attributes)];
        }

        $constructor = (new ReflectionClass($class))->getConstructor();

        $parameterNames = $constructor
            ? lp_collect($constructor->getParameters())->map->getName()->all()
            : [];

        return lp_collect($attributes)->partition(function ($value, $key) use ($parameterNames) {
    ...
    protected function getAttributesFromAttributeString(string $attributeString)
    {
        ...
        return lp_collect($matches)->mapWithKeys(function ($match) {
    ...
    {
        return lp_collect(token_get_all($value))->map(function ($token) {
            if (!is_array($token)) {
                return $token;
            }
    ...
    {
        return lp_collect($attributes)
            ->map(function (string $value, string $attribute) use ($escapeBound) {
                return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && !is_numeric($value)
                    ? "'{$attribute}' => \LaraWP\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                    : "'{$attribute}' => {$value}";
            })
            ->implode(',');
    }

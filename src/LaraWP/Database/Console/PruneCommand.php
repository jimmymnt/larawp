...
use LaraWP\Database\Eloquent\MassPrunable;
use LaraWP\Database\Eloquent\Prunable;
use LaraWP\Database\Eloquent\SoftDeletes;
use LaraWP\Database\Events\ModelsPruned;
...
class PruneCommand extends Command
{
    ...
    protected function models()
    {
        if (!empty($models = $this->option('model'))) {
            return lp_collect($models)->filter(function ($model) {
        ...
        return lp_collect((new Finder)->in($this->getDefaultPath())->files()->name('*.php'))
            ->map(function ($model) {
                ...
                return $namespace . str_replace(
                        ['/', '.php'],
                        ['\\', ''],
                        Str::after($model->getRealPath(), realpath(lp_app_path()) . DIRECTORY_SEPARATOR)
                    );
    ...
    {
        $uses = lp_class_uses_recursive($model);
    ...
    protected function pretendToPrune($model)
    {
        ...
        $count = $instance->prunable()
            ->when(in_array(SoftDeletes::class, lp_class_uses_recursive(get_class($instance))), function ($query) {
                $query->withTrashed();
            })->count();

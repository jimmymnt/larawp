...
namespace LaraWP\Testing\Concerns;

use LaraWP\Database\QueryException;
use LaraWP\Foundation\Testing;
use LaraWP\Support\Arr;
use LaraWP\Support\Facades\Artisan;
use LaraWP\Support\Facades\DB;
use LaraWP\Support\Facades\ParallelTesting;
use LaraWP\Support\Facades\Schema;
...
trait TestDatabases
{
    ...
    protected function bootTestDatabase()
    {
        ...
        ParallelTesting::setUpTestCase(function ($testCase) {
            $uses = array_flip(lp_class_uses_recursive(get_class($testCase)));
    ...
    protected function switchToDatabase($database)
    {
        ...
        $default = lp_config('database.default');

        $url = lp_config("database.connections.{$default}.url");

...
use LaraWP\Http\Client\Events\ConnectionFailed;
use LaraWP\Http\Client\Events\RequestSending;
use LaraWP\Http\Client\Events\ResponseReceived;
...
class PendingRequest
{
     ...
     *
     * @var \LaraWP\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \LaraWP\Support\Collection|null
     */
     ...
     *
     * @var \LaraWP\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param \LaraWP\Http\Client\Factory|null $factory
     * @return void
     */
    public function __construct(Factory $factory = null)
    {
        ...
        $this->beforeSendingCallbacks = lp_collect([function (Request $request, array $options, PendingRequest $pendingRequest) {
    ...
    {
        return lp_tap($this, function ($request) use ($format) {
    ...
    {
        return lp_tap($this, function ($request) use ($headers) {
    ...
    {
        return lp_tap($this, function ($request) use ($username, $password) {
    ...
    {
        return lp_tap($this, function ($request) use ($token, $type) {
    ...
    {
        return lp_tap($this, function ($request) use ($userAgent) {
    ...
    {
        return lp_tap($this, function ($request) use ($cookies, $domain) {
            return $this->options = array_merge_recursive($this->options, [
    ...
    {
        return lp_tap($this, function ($request) {
    ...
    {
        return lp_tap($this, function ($request) use ($to) {
    ...
    {
        return lp_tap($this, function () use ($seconds) {
    ...
    {
        return lp_tap($this, function ($request) use ($options) {
    ...
    {
        return lp_tap($this, function () use ($callback) {
     ...
     * @return \LaraWP\Http\Client\Response
     */
    ...
    public function pool(callable $callback)
    {
        ...
        $requests = lp_tap(new Pool($this->factory), $callback)->getRequests();
    ...
    public function send(string $method, string $url, array $options = [])
    {
        ...
        return lp_retry($this->tries ?? 1, function () use ($method, $url, $options) {
            try {
                return lp_tap(new Response($this->sendRequest($method, $url, $options)), function ($response) {
    ...
    {
        return lp_collect($data)->map(function ($value, $key) {
    ...
    protected function makePromise(string $method, string $url, array $options = [])
    {
        return $this->promise = $this->sendRequest($method, $url, $options)
            ->then(function (MessageInterface $message) {
                return lp_tap(new Response($message), function ($response) {
                    $this->populateResponse($response);
                    $this->dispatchResponseReceivedEvent($response);
                });
            })
    ...
    {
        return lp_tap($handlerStack, function ($stack) {
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    lp_optional($this->factory)->recordRequestResponsePair(
    ...
    public function buildStubHandler()
    {
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? lp_collect())
                    ->map
    ...
    {
        return lp_tap($request, function (&$request) use ($options) {
    ...
    {
        $this->stubCallbacks = lp_collect($callback);
    ...
    {
        if ($dispatcher = lp_optional($this->factory)->getDispatcher()) {

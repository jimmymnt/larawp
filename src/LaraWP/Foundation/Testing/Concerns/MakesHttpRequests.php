...
use LaraWP\Contracts\Http\Kernel as HttpKernel;
use LaraWP\Cookie\CookieValuePrefix;
use LaraWP\Http\Request;
use LaraWP\Support\Str;
use LaraWP\Testing\LoggedExceptionCollection;
use LaraWP\Testing\TestResponse;
...
trait MakesHttpRequests
{
     ...
     * @param string $uri
     * @param array $headers
     * @return \LaraWP\Testing\TestResponse
     */
    ...
    {
        return lp_collect(array_merge($this->defaultHeaders, $headers))->mapWithKeys(function ($value, $name) {
    ...
    protected function prepareCookiesForRequest()
    {
        ...
        return lp_collect($this->defaultCookies)->map(function ($value, $key) {
            return lp_encrypt(CookieValuePrefix::create($key, lp_app('encrypter')->getKey()) . $value, false);
     ...
     * @return \LaraWP\Http\Response|\LaraWP\Testing\TestResponse
     */
    ...
    {
        return lp_tap(TestResponse::fromBaseResponse($response), function ($response) {

...
use LaraPress\Contracts\Support\Htmlable;
...
use LaraPress\Support\Traits\ReflectsClosures;
use LaraPress\View\Component;
...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    {
        return lp_collect(token_get_all($contents))
            ->pluck(0)
    ...
    public static function render($string, $data = [], $deleteCachedView = false)
    {
        ...
        return lp_tap($view->render(), function () use ($view, $deleteCachedView) {
            if ($deleteCachedView) {
                unlink($view->getPath());
            }
        });
     ...
     *
     * @param \LaraPress\View\Component $component
     * @return string
     */
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php if (\LaraPress\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php if (\LaraPress\Support\Facades\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else' . $name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\LaraPress\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\LaraPress\Support\Facades\Blade::check('{$name}')): ?>";
        });
    ...
    public function component($class, $alias = null, $prefix = '')
    {
        ...
        if (is_null($alias)) {
            $alias = Str::contains($class, '\\View\\Components\\')
                ? lp_collect(explode('\\', Str::after($class, '\\View\\Components\\')))->map(function ($segment) {
                    return Str::kebab($segment);
                })->implode(':')
                : Str::kebab(lp_class_basename($class));
        }
    ...
    public function aliasInclude($path, $alias = null)
    {
        ...
        $this->directive($alias, function ($expression) use ($path) {
            ...
            return "<?php echo \$__env->make('{$path}', {$expression}, \LaraPress\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>";
        });
    }
    ...
    {
        $this->setEchoFormat('lp_e(%s, true)');
    }
    ...
    {
        $this->setEchoFormat('lp_e(%s, false)');
    }

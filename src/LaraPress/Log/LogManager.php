...
class LogManager implements LoggerInterface
{
    ...
    {
        try {
            return $this->channels[$name] ?? lp_with($this->resolve($name, $config), function ($logger) use ($name) {
                return $this->channels[$name] = $this->tap($name, new Logger($logger, $this->app['events']));
            });
        } catch (Throwable $e) {
            return lp_tap($this->createEmergencyLogger(), function ($logger) use ($e) {
     ...
     *
     * @param string $name
     * @param \LaraPress\Log\Logger $logger
     * @return \LaraPress\Log\Logger
     */
    ...
    protected function createStackDriver(array $config)
    {
        ...
        $handlers = lp_collect($config['channels'])->flatMap(function ($channel) {
            return $channel instanceof LoggerInterface
                ? $channel->getHandlers()
                : $this->channel($channel)->getHandlers();
        })->all();

        $processors = lp_collect($config['channels'])->flatMap(function ($channel) {
            return $channel instanceof LoggerInterface
    ...
    {
        return lp_tap(new LineFormatter(null, $this->dateFormat, true, true), function ($formatter) {

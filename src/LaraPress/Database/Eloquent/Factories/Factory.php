...
abstract class Factory
{
     ...
     * @param \LaraPress\Support\Collection|null $states
     * @param \LaraPress\Support\Collection|null $has
     * @param \LaraPress\Support\Collection|null $for
     * @param \LaraPress\Support\Collection|null $afterMaking
     * @param \LaraPress\Support\Collection|null $afterCreating
     ...
     * @param \LaraPress\Database\Eloquent\Model|null $parent
     * @return array
     */
    ...
    public function createMany(iterable $records)
    {
        return new EloquentCollection(
            lp_collect($records)->map(function ($record) {
     ...
     * @param array $attributes
     * @param \LaraPress\Database\Eloquent\Model|null $parent
     * @return \LaraPress\Database\Eloquent\Collection|\LaraPress\Database\Eloquent\Model
     */
    public function create($attributes = [], ?Model $parent = null)
    {
        ...
        if ($results instanceof Model) {
            $this->store(lp_collect([$results]));

            $this->callAfterCreating(lp_collect([$results]), $parent);
        } else {
            $this->store($results);
     ...
     *
     * @param \LaraPress\Support\Collection $results
     * @return void
     */
    ...
    public function make($attributes = [], ?Model $parent = null)
    {
        ...
        if ($this->count === null) {
            return lp_tap($this->makeInstance($parent), function ($instance) {
                $this->callAfterMaking(lp_collect([$instance]));
            });
        }
    ...
    protected function makeInstance(?Model $parent)
    {
        return ModelAbstract::unguarded(function () use ($parent) {
            return lp_tap($this->newModel($this->getExpandedAttributes($parent)), function ($instance) {
                if (isset($this->connection)) {
    ...
    {
        return lp_collect($definition)->map(function ($attribute, $key) use (&$definition) {
    ...
    {
        $guess = Str::camel(Str::plural(lp_class_basename($related)));
    ...
    public function hasAttached($factory, $pivot = [], $relationship = null)
    {
        return $this->newInstance([
            'has' => $this->has->concat([new BelongsToManyRelationship(
                $factory,
                $pivot,
                $relationship ?: Str::camel(Str::plural(lp_class_basename(
    ...
    public function for($factory, $relationship = null)
    {
        return $this->newInstance(['for' => $this->for->concat([new BelongsToRelationship(
            $factory,
            $relationship ?: Str::camel(lp_class_basename(
     ...
     *
     * @param \LaraPress\Support\Collection $instances
     * @return void
     */
    ...
    public function modelName()
    {
        $resolver = static::$modelNameResolver ?: function (self $factory) {
            ...
            $factoryBasename = Str::replaceLast('Factory', '', lp_class_basename($factory));

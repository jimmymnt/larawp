...
use Closure;
use LaraPress\Contracts\Mail\Factory;
use LaraPress\Contracts\Mail\Mailable;
use LaraPress\Contracts\Mail\Mailer;
use LaraPress\Contracts\Mail\MailQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = lp_collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = lp_collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
    ...
    {
        return lp_collect($this->mailables)->filter(function ($mailable) use ($type) {
    ...
    {
        return lp_collect($this->queuedMailables)->filter(function ($mailable) use ($type) {
            return $mailable instanceof $type;
        });
    }

...
use LaraPress\Contracts\Bus\QueueingDispatcher;
...
class BusFake implements QueueingDispatcher
{
     ...
     *
     * @var \LaraPress\Contracts\Bus\QueueingDispatcher
     */
     ...
     *
     * @param \LaraPress\Contracts\Bus\QueueingDispatcher $dispatcher
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(
            lp_collect($expectedChain)->isNotEmpty(),
    ...
    {
        return lp_tap(clone $job, function ($job) {
    ...
    {
        $chain = lp_collect($expectedChain)->map(function ($job) {
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return lp_collect($chain)->map(function ($job) {
    ...
    {
        return !lp_collect($chain)->contains(function ($job) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return lp_collect($this->commands[$command])->filter(function ($command) use ($callback) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return lp_collect($this->commandsSync[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return lp_collect($this->commandsAfterResponse[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return lp_collect($this->batches)->filter(function ($batch) use ($callback) {
     ...
     *
     * @param \LaraPress\Bus\PendingBatch $pendingBatch
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return lp_collect($this->jobsToFake)

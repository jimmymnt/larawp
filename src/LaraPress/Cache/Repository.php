...
use LaraPress\Cache\Events\CacheHit;
use LaraPress\Cache\Events\CacheMissed;
use LaraPress\Cache\Events\KeyForgotten;
use LaraPress\Cache\Events\KeyWritten;
...
/**
 * @mixin \LaraPress\Contracts\Cache\Store
 */
class Repository implements ArrayAccess, CacheContract
{
     ...
     *
     * @var \LaraPress\Contracts\Cache\Store
     */
    protected $store;
     ...
     *
     * @param \LaraPress\Contracts\Cache\Store $store
     * @return void
     */
    ...
    {
        $values = $this->store->many(lp_collect($keys)->map(function ($value, $key) {
            return is_string($key) ? $key : $value;
        })->values()->all());

        return lp_collect($values)->map(function ($value, $key) use ($keys) {
    ...
    protected function handleManyResult($keys, $key, $value)
    {
        ...
        if (is_null($value)) {
            ...
            return isset($keys[$key]) ? lp_value($keys[$key]) : null;
        }
    ...
    {
        return lp_tap($this->get($key, $default), function () use ($key) {
            $this->forget($key);
        });
    }
    ...
    public function remember($key, $ttl, Closure $callback)
    {
        ...
        $this->put($key, $value = $callback(), lp_value($ttl));
    ...
    {
        return lp_tap($this->store->forget($this->itemKey($key)), function ($result) use ($key) {
            if ($result) {
     ...
     *
     * @return \LaraPress\Contracts\Cache\Store
     */

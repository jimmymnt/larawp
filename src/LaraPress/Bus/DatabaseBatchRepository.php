...
use LaraPress\Database\PostgresConnection;
...
class DatabaseBatchRepository implements PrunableBatchRepository
{
     ...
     *
     * @var \LaraPress\Bus\BatchFactory
     */
    protected $factory;
     ...
     *
     * @param \LaraPress\Bus\BatchFactory $factory
     ...
     *
     * @param \LaraPress\Bus\PendingBatch $batch
     * @return \LaraPress\Bus\Batch
     */
    ...
    protected function updateAtomicValues(string $batchId, Closure $callback)
    {
        return $this->connection->transaction(function () use ($batchId, $callback) {
            ...
            return is_null($batch) ? [] : lp_tap($callback($batch), function ($values) use ($batchId) {
